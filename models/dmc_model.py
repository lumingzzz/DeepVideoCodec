...
class CheckerboardContextModel(CompressionModel):
...
class DMC(nn.Module):
    def __init__(self):
        ...
        self.mv_context_model = CheckerboardContextModel(planes=channel_mv, latent_planes=channel_N)
        ...
        self.frame_context_model = CheckerboardContextModel(planes=channel_M, latent_planes=channel_N)
    ...
    def forward(self, frames, motion_pre=False, frame_pre=False):
        ...
        for i in range(1, len(frames)):
            x = frames[i]
            x_rec, likelihoods, context = self.forward_inter(x, dpb, motion_pre, frame_pre)
            ...
            if len(frames) >= 3:
                dpb = {
                        ...
                        "y_ref": context["y_ref"],
                        "y_ref_motion": context["y_ref_motion"],
                    }
    ...
    def forward_inter(self, x_cur, dpb, motion_pre=False, frame_pre=False):
        x_ref = dpb["x_ref"]

        motion = self.optic_flow(x_cur, x_ref)
        y_motion = self.mv_encoder(motion)
        y_motion_hat, motion_likelihoods = self.mv_context_model(y_motion, dpb["y_ref_motion"])
        ...
        if motion_pre:
            ...
            for param in self.optic_flow.parameters():
                param.requires_grad = False
            for param in self.mv_encoder.parameters():
                param.requires_grad = False
            for param in self.mv_decoder.parameters():
                param.requires_grad = False
            context1, context2, context3, x_warp = context1.detach(), context2.detach(), context3.detach(), x_warp.detach()
        
        y = self.contextual_encoder(x_cur, context1, context2, context3)
        y_hat, frame_likelihoods = self.frame_context_model(y, dpb["y_ref"])
        ...
        return x_rec, \
               {"motion": motion_likelihoods, "frame": frame_likelihoods}, \
               {"x_ref": x_rec, "feature_ref": feature, "y_ref": y_hat, "y_ref_motion": y_motion_hat}
