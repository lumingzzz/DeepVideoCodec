...
def mse_loss(fake, real):
    ...
    return F.mse_loss(fake, real, reduction='none').mean(dim=(1,2,3))
...
class VariableRateLossyVAE(nn.Module):
    ...
    def forward(self, batch, log_lmb=None, return_rec=False):
        ...
        with torch.no_grad():
            ...
            im_hat = x_hat.detach()
            im_mse = F.mse_loss(im_hat, im, reduction='mean')
    ...
    #         im = F.to_tensor(Image.open(impath)).unsqueeze_(0).to(device=self._dummy.device)
    ...
    def self_evaluate(self, im, log_lmb: float):
        ...
        mse = F.mse_loss(im, x_hat, reduction='mean').item()
...
def vr_intra(lmb_range=[128,1024]):
    cfg = dict()
