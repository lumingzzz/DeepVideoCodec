...
from lvae.models.common import ME_Spynet, bilineardownsacling, flow_warp
import lvae.models.video.video_model as qrvm
from lvae.models.vr.library import vr_qres
from lvae.models.video import mv_codec
...
class ConditionalLossyVAE(nn.Module):
    ...
    def __init__(self, distortion_lmb):
        ...
        self.mv_codec = mv_codec()

    def motion_compensation(self, dpb, mv):
        warp_frame = flow_warp(dpb["ref_frame"], mv)
        ...
        return context1, context2, context3, warp_frame

    def forward_end2end(self, im, dpb, log_lmb=None, get_intermediate=False):
        im_prev = dpb[]
        est_mv = self.optic_flow(im, im_prev)
        mv_hat, flow_stats = self.mv_codec.forward_end2end(est_mv, log_lmb)
        context1, context2, context3, warp_frame = self.motion_compensation(mv_hat, dpb)
...
class VideoModel(nn.Module):
    def __init__(self, lmb=2048):
        ...
        # ================================ i-frame model ================================
        i_lambda = lmb // 2
        self.i_model = vr_qres(pretrained=True)
        self.i_model._default_log_lmb = i_lambda
        ...
        # ================================ p-frame model ================================
        self.p_model = ConditionalLossyVAE(lmb)
